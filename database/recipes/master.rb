...
if node[:database][:s3_backup]
  Chef::Log.info "Setting up templates for chef-solo s3 backups"
  template "/etc/chef/chef-solo-database-s3_backup.rb" do
    source "chef-solo-database-s3_backup.rb.erb"
  ...
  template "/etc/chef/chef-solo-database-s3_backup.json" do
    source "chef-solo-database-s3_backup.json.erb"
    variables(
      :output => {
        's3_backup' => {
          ...
          'bucket' => node[:database][:s3_backup][:bucket],
          'databases' => databases_to_backup,
        },
        'run_list' => [
          "recipe[database::s3_backup]"
        ]
      }
    )
    owner "root"
  ...
  template "/etc/cron.d/chef-solo-database-s3_backup" do
    source "chef-solo-database-s3_backup.cron.erb"
    variables(
      :json_attribs => "/etc/chef/chef-solo-database-s3_backup.json",
      :config_file => "/etc/chef/chef-solo-database-s3_backup.rb"
    )
    owner "root"
    group "root"
    mode 0600
