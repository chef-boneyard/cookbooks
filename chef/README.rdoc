...
This cookbook no longer manages the chef package version, it manages /etc/chef/client.rb, reloads the configuration if the template changes, and can auto-validate a client if /etc/chef/validation_token exists.
...
By default, the server is setup to run as a standard Merb application with the Mongrel adapter, using the package installation or the bootstrap cookbook. The chef::server recipe can be used to maintain the configuration.

Optionally, the chef::server_proxy recipe can be added to provide SSL encryption. If you use this, clients should have "url_type" set to "https" so the right URLs are used in the configuration.
